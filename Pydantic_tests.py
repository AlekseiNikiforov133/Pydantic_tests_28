...
def test_users_get_response():
    response = [
        {"id": 123, "first_name": "Pingvin", "last_name": "Goober"},
        {"id": 456, "first_name": "Kumikon", "last_name": "Goober"}
    ]
...
def test_users_get_success():
    ...
    assert users[0].first_name == "Pingvin"
    assert users[0].last_name == "Goober"
...
def test_users_get_one_user():
    response = [{"id": 123, "first_name": "Pingvin", "last_name": "Goober"}]
    users = [User(**user) for user in response]
    assert len(users) == 1
    assert users[0].id == 123
    assert users[0].first_name == "Pingvin"
